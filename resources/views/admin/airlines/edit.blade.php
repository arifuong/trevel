<x-layouts.admin title="Edit Maskapai">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-6" style="animation: fadeSlideUp 0.4s ease both; animation-delay: 0.05s;">
            <a href="{{ route('admin.airlines.index') }}" class="text-[#526057] hover:text-[#122B1F] text-sm flex items-center mb-4 transition-colors">
                ...
                Kelola Maskapai &gt; Edit Maskapai
            </a>
            <h1 class="text-2xl font-bold text-[#122B1F]">Edit Maskapai</h1>
            <p class="text-[#526057] text-sm mt-1">Perbarui data maskapai penerbangan.</p>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-[#E0E7DC] shadow-xs overflow-hidden" style="animation: fadeSlideUp 0.4s ease both; animation-delay: 0.15s;" x-data="{
            photoPreview: '{{ $airline->logo ? Storage::url($airline->logo) : '' }}',
            updatePreview(event) {
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="col-span-2 md:col-span-1">
                                ...
                                <input type="text" id="name" name="name" value="{{ old('name', $airline->name) }}" required
                            ...
                            <div class="col-span-2 md:col-span-1">
                                ...
                                <input type="text" id="code" name="code" value="{{ old('code', $airline->code) }}"
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div class="col-span-1">
                                <label class="block text-sm font-medium text-[#122B1F] mb-2">Logo (Upload baru untuk mengganti)</label>
                                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-[#E0E7DC] border-dashed rounded-xl relative overflow-hidden bg-[#F8FAF7]" :class="photoPreview ? 'p-2 border-solid' : ''">
                                    ...
                                    <template x-if="photoPreview">
                                        <div class="relative w-full max-w-sm mx-auto group flex items-center justify-center min-h-[140px]">
                                            ...
                                            <div class="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity rounded-xl">
                                                <label for="logo" class="cursor-pointer bg-white text-[#122B1F] rounded-lg px-3 py-1.5 text-xs font-semibold shadow-sm hover:bg-gray-50">
                                                    Ganti Logo
                                                </label>
                                    ...
                                    <div class="space-y-1 text-center" x-show="!photoPreview">
                                        ...
                                        <div class="flex text-sm text-[#526057] justify-center">
                                            <label for="logo_empty" class="relative cursor-pointer bg-white rounded-md font-medium text-[#1B3B2B] hover:text-[#122B1F] focus-within:outline-none">
                                                <span>Upload Foto</span>
                                                <input id="logo_empty" name="logo" type="file" accept="image/jpeg,image/png,image/webp,image/jpg" class="sr-only" @change="updatePreview">
                                            </label>
                                        </div>
