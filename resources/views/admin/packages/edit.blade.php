<x-layouts.admin title="Edit Paket">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-6" style="animation: fadeSlideUp 0.4s ease both; animation-delay: 0.05s;">
            ...
            <h1 class="text-2xl font-bold text-[#122B1F]">Edit Paket: {{ $package->name }}</h1>
            <p class="text-[#526057] text-sm mt-1">Ubah informasi dasar paket umroh utama.</p>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-[#E0E7DC] shadow-xs overflow-hidden" style="animation: fadeSlideUp 0.4s ease both; animation-delay: 0.15s;" x-data="{
            photoPreview: '{{ $package->main_photo ? Storage::url($package->main_photo) : '' }}',
            updatePreview(event) {
            ...
            <form action="{{ route('admin.packages.update', $package) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="p-6 space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="col-span-2 md:col-span-1">
                                ...
                                <select id="package_type" name="package_type" class="w-full rounded-xl border border-[#E0E7DC] text-sm px-4 py-2.5 focus:ring-[#1B3B2B] focus:border-[#1B3B2B] outline-none transition-colors bg-white">
                                    <option value="umrah" {{ old('package_type', $package->package_type) == 'umrah' ? 'selected' : '' }}>Umrah</option>
                                    <option value="haji" {{ old('package_type', $package->package_type) == 'haji' ? 'selected' : '' }}>Haji</option>
                                </select>
                            </div>

                            <div class="col-span-2 md:col-span-1">
                                <label for="category_label" class="block text-sm font-medium text-[#122B1F] mb-1">Label Kategori (Opsional)</label>
                                <input type="text" id="category_label" name="category_label" value="{{ old('category_label', $package->category_label) }}" placeholder="Contoh: Umrah Kemerdekaan, Haji Mujamalah"
                                    class="w-full rounded-xl border border-[#E0E7DC] text-sm px-4 py-2.5 focus:ring-[#1B3B2B] focus:border-[#1B3B2B] outline-none transition-colors">
                            ...
                            <div class="col-span-2 md:col-span-1">
                                ...
                                <div class="relative">
                                    <input type="number" id="duration" name="duration" value="{{ old('duration', $package->duration) }}" required min="1"
                            ...
                            <div class="col-span-2 md:col-span-1">
                                ...
                                <input type="date" id="departure_date" name="departure_date" value="{{ old('departure_date', $package->departure_date ? \Carbon\Carbon::parse($package->departure_date)->format('Y-m-d') : '') }}" required
                                    class="w-full rounded-xl border border-[#E0E7DC] text-sm px-4 py-2.5 focus:ring-[#1B3B2B] focus:border-[#1B3B2B] outline-none transition-colors">
                            </div>
                            ...
                            <div class="col-span-2">
                                ...
                                <textarea id="description" name="description" rows="4" 
                                    class="w-full rounded-xl border border-[#E0E7DC] text-sm px-4 py-2.5 focus:ring-[#1B3B2B] focus:border-[#1B3B2B] outline-none transition-colors">{{ old('description', $package->description) }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <!-- Foto Utama -->
                            <div class="col-span-2 md:col-span-1">
                                <label class="block text-sm font-medium text-[#122B1F] mb-2">Foto Utama</label>
                                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-[#E0E7DC] border-dashed rounded-xl relative overflow-hidden bg-[#F8FAF7]" :class="photoPreview ? 'p-2 border-solid' : ''">
                                    ...
                                    <div class="space-y-1 text-center" x-show="!photoPreview">
                                        <svg class="mx-auto h-12 w-12 text-[#526057]" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                                        ...
                                        <p class="text-xs text-[#526057]">Kosongkan jika tidak ingin mengubah</p>
                ...
                <div class="px-6 pb-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6" x-data="{
                        includes: {{ json_encode($package->includes->pluck('item')->toArray() ?: ['Tiket Pesawat PP', 'Visa Umrah', 'Hotel Makkah & Madinah', 'Makan 3x Sehari', 'Handling Bandara']) }},
                        excludes: {{ json_encode($package->excludes->pluck('item')->toArray() ?: ['Pembuatan Paspor', 'Vaksin Meningitis & Polio', 'Pengeluaran Pribadi (Laundry/Telepon)', 'Kelebihan Bagasi']) }}
                    }">
                        <!-- Biaya Termasuk -->
