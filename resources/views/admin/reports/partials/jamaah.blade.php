...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-4 mb-5" style="animation: fadeSlideUp 0.3s ease both">
    <div class="bg-white rounded-2xl border border-[#E0E7DC] p-4 flex items-center justify-between shadow-2xs">
        <div>
            <span class="text-[10px] sm:text-[11px] text-[#526057] uppercase tracking-wider block font-bold">Pendaftaran Baru</span>
            <span class="text-2xl font-extrabold text-[#12271E] mt-0.5 block leading-tight">{{ $summary['totalRegistrations'] }}</span>
            <span class="text-[10px] text-[#526057]/70 block mt-0.5">Booking dibuat</span>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-[#CCD8C7] p-4 flex items-center justify-between shadow-2xs">
        <div>
            <span class="text-[10px] sm:text-[11px] text-[#1B3B2B] uppercase tracking-wider block font-bold">Total Jamaah (Pax)</span>
            <span class="text-2xl font-extrabold text-[#1B3B2B] mt-0.5 block leading-tight">{{ $summary['totalPax'] }}</span>
            <span class="text-[10px] text-[#526057]/70 block mt-0.5">Termasuk anggota keluarga</span>
        </div>
        <div class="w-8 h-8 rounded-xl bg-emerald-50 text-emerald-800 flex items-center justify-center shrink-0">
    ...
    <div class="bg-white rounded-2xl border border-emerald-200 p-4 flex items-center justify-between shadow-2xs">
        <div>
            <span class="text-[10px] sm:text-[11px] text-emerald-800 uppercase tracking-wider block font-bold">Nilai Kontrak Paket</span>
            <span class="text-xl sm:text-2xl font-extrabold text-emerald-950 mt-0.5 block leading-tight">
                Rp {{ number_format($summary['totalInvoiceValue'], 0, ',', '.') }}
            </span>
            <span class="text-[10px] text-[#526057]/70 block mt-0.5">Omzet pendaftaran</span>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-blue-200 p-4 flex items-center justify-between shadow-2xs">
        <div>
            <span class="text-[10px] sm:text-[11px] text-blue-800 uppercase tracking-wider block font-bold">Jamaah Resmi / Berangkat</span>
            <span class="text-2xl font-extrabold text-blue-950 mt-0.5 block leading-tight">{{ $summary['activePaxCount'] }}</span>
            <span class="text-[10px] text-[#526057]/70 block mt-0.5">Tahap jamaah s/d selesai</span>
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
            ...
            <div class="relative flex-1">
                <input type="text" 
                       ...
                       placeholder="Cari nomor pendaftaran, nama pemesan, paket, atau no. hp..."
            ...
            {{-- Filter Status Pendaftaran (Auto-Apply on Change) --}}
            <div class="flex items-center gap-1.5 sm:w-64">
                ...
                <select x-model="status" 
                    ...
                    <option value="menunggu_verifikasi_dokumen">Verifikasi Dokumen</option>
                    <option value="menunggu_pembayaran_dp">Menunggu Bayar DP</option>
                    <option value="menunggu_verifikasi_pembayaran_dp">Verifikasi DP</option>
                    <option value="jamaah">Jamaah Resmi</option>
                    <option value="cicilan_pelunasan">Cicilan Pelunasan</option>
                    <option value="lunas">Lunas</option>
                    <option value="menunggu_kelengkapan_keberangkatan">Dokumen Keberangkatan</option>
        ...
        <table class="w-full text-left text-xs sm:text-sm">
            <thead class="bg-[#EFF3EB] text-[#12271E] uppercase text-[10px] sm:text-[11px] font-bold tracking-wider border-b border-[#E0E7DC]">
                <tr>
                    ...
                    <th class="py-3 px-4">Paket Pilihan</th>
                    ...
                    <tr class="hover:bg-[#F8FAF7] transition-colors">
                        ...
                        <td class="py-3.5 px-4">
                            ...
                            <div class="text-[11px] text-[#526057]">
                                {{ $reg->packageVariant?->name ? 'Varian ' . $reg->packageVariant->name . ' • ' : '' }}
                                Kamar {{ ucfirst($reg->room_type ?? 'Quad') }}
                            </div>
                        </td>
                        <td class="py-3.5 px-4 text-center whitespace-nowrap">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-[#EFF3EB] text-[#12271E]">
                        ...
                        <td class="py-3.5 px-4 text-right whitespace-nowrap text-emerald-700 font-semibold">
                            Rp {{ number_format($reg->invoice->total_paid ?? 0, 0, ',', '.') }}
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="8" class="py-12 text-center text-[#526057]">
                            ...
                            <p class="font-semibold text-sm text-[#12271E]">Tidak Ada Data Pendaftaran</p>
                            <p class="text-xs mt-1">Tidak ditemukan pendaftaran pada periode {{ $periodData['label'] }}.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
