...
<body class="font-sans antialiased bg-[#F7F5F0] text-[#526057] min-h-screen flex flex-col selection:bg-[#1B3B2B] selection:text-white">
                ...
                <div class="flex items-center gap-2 sm:gap-6 min-w-0">
                    ...
                    <nav class="hidden md:flex items-center space-x-1 pl-3 border-l border-white/10 text-xs">
                        ...
                        <a href="{{ route('jamaah.history') }}" 
                           class="px-3.5 py-2 rounded-full font-semibold transition-all {{ request()->routeIs('jamaah.history*') ? 'bg-white/15 text-white shadow-2xs' : 'text-white/75 hover:text-white hover:bg-white/10' }}">
                            Riwayat
                        </a>
    ...
    <nav class="md:hidden fixed bottom-0 inset-x-0 z-50 bg-white/95 backdrop-blur-lg border-t border-[#E0E7DC] shadow-[0_-8px_20px_rgba(27,59,43,0.06)]"
        ...
        <div class="grid grid-cols-5 items-center h-16 px-1.5 max-w-md mx-auto relative">
            ...
            @php $isHistory = request()->routeIs('jamaah.history*'); @endphp
            <a href="{{ route('jamaah.history') }}" 
               class="flex flex-col items-center justify-center h-full min-h-[44px] py-1 text-center transition-all group {{ $isHistory ? 'text-[#1B3B2B]' : 'text-[#526057] hover:text-[#1B3B2B]' }}"
               aria-label="Riwayat Perjalanan">
                <div class="relative p-1 rounded-xl transition-transform duration-200 {{ $isHistory ? 'scale-110' : 'group-active:scale-95' }}">
                    <svg class="w-5 h-5 {{ $isHistory ? 'stroke-[2.5]' : 'stroke-2' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <span class="text-[10px] tracking-tight mt-0.5 {{ $isHistory ? 'font-bold text-[#1B3B2B]' : 'font-medium text-[#526057]' }}">
                    Riwayat
                </span>
            </a>
